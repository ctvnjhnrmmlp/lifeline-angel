...
with open('./dataset-dirty.json') as file:
...
def json_to_plain_text(data):
    ...
    for intent in data["intents"]:
        ...
        # plain_text += f"Type: {intent['type']}\n"
        # plain_text += f"Meaning: {intent['meaning']}\n"
        plain_text += "Patterns:\n"
        ...
        for link in intent['links']:
            plain_text += f"{link}\n"
        plain_text += "\n"
        # plain_text += "Relations:\n"
        # for relation in intent['relations']:
            # plain_text += f"{relation}\n"
        # plain_text += "References:\n"
        # for reference in intent['references']:
        #     plain_text += f"{reference}\n"
        # plain_text += f"Context Set: {intent['context_set']}\n\n"
    return plain_text
...
with open("./dataset-dirty.txt", "w") as text_file:
